...
const VALID_STATUSES = ["open", "responded", "dismissed", "escalated"] as const;
...
export async function PATCH(req: NextRequest, { params }: RouteContext) {
  ...
  // Build the update patch from whichever fields the caller sent.
  ...
  if (error || !data) {
    return NextResponse.json(
      { error: error?.message ?? "Could not update concern." },
      { status: 500 },
    );
  }
